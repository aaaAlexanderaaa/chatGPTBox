...
export function applyResumePatch(target, operation = {}) {
  ...
  if (pathSegments.length === 0 || pathSegments.some((part) => FORBIDDEN_PATCH_SEGMENTS.has(part))) {
    return false
  }
...
export function createChatgptWebResumeDeltaAccumulator() {
  ...
  function feedEvent(eventName, payload) {
    ...
    if (!eventName) {
      ...
      else if (payload.type === 'input_message') inputMessage = payload.input_message || inputMessage
      return false
    }
  ...
  await fetchSSE(url, {
    ...
    onEvent(event) {
      ...
      if (payload.conversation_id && payload.message) {
        onMessageSnapshot?.({ conversation_id: payload.conversation_id, message: payload.message })
      }
      ...
      onMessageSnapshot?.({ conversation_id: body.conversation_id, message: bestMessage.message })
    },
  })
